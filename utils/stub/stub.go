...
import (
	...
	"path/filepath"
	"strings"
	"github.com/mileapp_screening/utils/modname"
	"text/template"
...
func MakeStubs() error {
	...
	for _, f := range files {
		...
		output := bytes.Replace(input, []byte("github.com/mileapp_screening/"), []byte("github.com/mileapp_screening/"), -1)
		output = bytes.Replace(output, []byte("module github.com/mileapp_screening"), []byte("module github.com/mileapp_screening"), -1)
...
func TemplateStub(templateType string, templateName string, name string) {
	contentsBuff, err := os.ReadFile(fmt.Sprintf("./stubs/github.com/mileapp_screening/%s/%s.go.stuba", templateType, templateName))
	if err != nil {
